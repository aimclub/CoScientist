...
def upload_and_ref(
    ...
    """Upload one file and return the durable ``(bucket, key)``, not a URL.

    :func:`upload_and_presign` answers with a presigned URL, which is the right
    shape for markdown a browser opens now and the wrong one for anything the
    report must still resolve next week: the signature expires, the object does
    not. Callers that record a reference — the NIR report's DOCX, say — need the
    pair, and mint a fresh URL from it on each request through
    ``/api/artifact/<bucket>/<key>``.

    Returns None on any failure, like everything else in this module.
    """
    try:
        path = Path(local_path)
        if not path.is_file():
