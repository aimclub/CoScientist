...
const i18n = {
  ...
  'settings.group.alembic': { en: 'Alembic MCP servers', ru: 'MCP-серверы Alembic' },
  'settings.group.alembic.desc': {
    en: 'How an agent obtains a tool server from a code repository: a local build, then the public hub, then a new conversion.',
    ru: 'Как агент получает сервер инструментов из репозитория: локальная сборка, затем публичный хаб, затем новая конверсия.'
  },
  ...
  // Fields — Alembic
  'settings.f.hubSearch.label': { en: 'Look in the public hub', ru: 'Искать в публичном хабе' },
  'settings.f.hubSearch.desc': {
    en: 'Before converting a repository, check the Docker Hub catalogue for a server already built from it and pull it instead.',
    ru: 'Перед конверсией репозитория проверить каталог на Docker Hub: если сервер из него уже собран, скачать его вместо сборки.'
  },
  'settings.f.hubAutoUpload.label': { en: 'Upload finished builds', ru: 'Выгружать готовые сборки' },
  'settings.f.hubAutoUpload.desc': {
    en: 'Push every finished build to the hub. A build with fewer passing tools never replaces a better one; the manual button always does.',
    ru: 'Отправлять каждую готовую сборку в хаб. Сборка с меньшим числом рабочих инструментов не заменит лучшую; ручная кнопка заменяет всегда.'
  },
  'settings.f.agentBuild.label': { en: 'Agents may convert repositories', ru: 'Агентам можно конвертировать репозитории' },
  'settings.f.agentBuild.desc': {
    en: 'Let an agent start a new conversion when neither the host nor the hub has a server for the repository. Off: agents only reuse.',
    ru: 'Разрешить агенту запускать новую конверсию, когда сервера нет ни локально, ни в хабе. Выключено: агенты только переиспользуют.'
  },
