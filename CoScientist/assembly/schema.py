...
def load_config(path: Optional[Path] = None) -> SystemConfig:
    ...
    if dataset.provider == "masda":
        ...
        agents[name] = {
            "class": "masda_a2a",
            "description": (
                "MASDA_Datasets remote A2A provider: acquires and processes existing "
                "external datasets for downstream CoScientist agents."
            ),
            "routing": (
                "for acquiring an existing external dataset from a natural-language "
                "request; materialize its embedded CSV in the current CoScientist "
                "workspace and report the real local path. Do not use for data "
