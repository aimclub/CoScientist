...
The YAML declares every agent of the system in one place. Per agent:
  ...
  class:        llm | sequential | parallel | masda_a2a | custom:<registered name>
  ...
  masda_a2a:    experimental external MASDA transport; never served locally
...
class AgentConfig(BaseModel):
    ...
    def _known_class(cls, v: str) -> str:
        if v in ("llm", "sequential", "parallel", "masda_a2a") or v.startswith("custom:"):
            return v
        raise ValueError(
            f"class must be llm | sequential | parallel | masda_a2a | custom:<name>, got {v!r}"
        )
    ...
    def _shape(self) -> "AgentConfig":
        ...
        if self.cls == "masda_a2a":
            if self.tools or self.subordinates or self.prompt or self.a2a:
                raise ValueError("masda_a2a cannot declare local tools, subordinates, prompt or serving config")
            if not (self.options.get("rpc_url") or self.options.get("card_url")):
                raise ValueError("masda_a2a requires options.rpc_url or options.card_url")
        return self
...
def load_config(path: Optional[Path] = None) -> SystemConfig:
    ...
    if dataset.provider == "masda":
        if not dataset.masda_a2a_version.strip():
            raise ValueError("DATASET__MASDA_A2A_VERSION must not be empty")
        if dataset.masda_timeout_s <= 0:
            raise ValueError("DATASET__MASDA_TIMEOUT_S must be positive")
        rpc_url = (dataset.masda_a2a_rpc_url or "").strip()
        card_url = (dataset.masda_a2a_card_url or "").strip()
        if not (rpc_url or card_url):
            raise ValueError(
                "DATASET__PROVIDER=masda requires DATASET__MASDA_A2A_RPC_URL "
                "or DATASET__MASDA_A2A_CARD_URL"
            )
        for variable, url in (
            ("DATASET__MASDA_A2A_RPC_URL", rpc_url),
            ("DATASET__MASDA_A2A_CARD_URL", card_url),
        ):
            if url:
                parsed = urlsplit(url)
        ...
        agents = raw["agents"]
        name = "MasdaDatasetsAgent"
        if name in agents:
            raise ValueError(f"{name} is reserved for the MASDA dataset provider")
        agents[name] = {
            "class": "masda_a2a",
            "description": (
                "MASDA_Datasets acquisition provider for external tabular data; "
                "materializes a CoScientist workspace CSV artifact."
            ),
            "routing": (
                "for acquiring external tabular data absent from local files and "
                "user uploads. Exact direct CSV URLs are validated v1; description-only "
                "discovery is experimental. Do not use for analysis or generated data."
            ),
            "options": {
                "rpc_url": rpc_url or None,
                "card_url": card_url or None,
                "a2a_version": dataset.masda_a2a_version,
                "timeout_s": dataset.masda_timeout_s,
            },
        }
        agents["OrchestratorAgent"]["subordinates"].append(name)
        coder = agents["CoderAgent"]
        coder["subordinates"] = [
            sub for sub in coder.get("subordinates", [])
            if sub != "DatasetCollectorAgent"
        ]
        agents["DatasetCollectorAgent"]["enabled"] = False
