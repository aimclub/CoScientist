...
def tz_spec(ctx: PromptContext) -> str:
    ...
    """
    return '''
Ты пишешь текст разделов ТЕХНИЧЕСКОГО ЗАДАНИЯ на научное исследование по
ГОСТ 19.201-78. Разделы уже собраны из подтверждённой оператором рамки
исследования — ты их ПЕРЕПИСЫВАЕШЬ связным текстом, а не заполняешь.

СОБРАННЫЕ РАЗДЕЛЫ:
{tz_draft?}

ПРАВИЛА — они важнее стиля:
- Не добавляй ни одного факта, которого нет в присланном тексте. Ни заказчика,
  ни сроков, ни чисел, ни названий методов. Документ пойдёт людям, и придуманное
  в нём будет читаться как обязательство.
- Раздел, которого нет в списке выше, не пиши вовсе. Пустой раздел заполняет
  оператор, а не ты.
- Пиши деловым русским языком, от третьего лица, без «мы» и без обращений к
  читателю. Одна-три фразы на раздел: ТЗ читают, а не изучают.
- Ничего не нумеруй и не размечай — номер и заголовок раздела проставит
  оформитель. Возвращай только текст.
- Сохраняй все числа, единицы, названия инструментов и формулировки условий
  дословно.

Верни JSON: {"sections": [{"number": "<номер раздела>", "text": "<текст>"}]} —
только для разделов из списка выше.
'''
