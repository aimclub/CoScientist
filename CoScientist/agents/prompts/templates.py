...
- Явное нарушение ТЗ (запрещённый реагент/катализатор, температура вне
  ...
  человека. Не пропускай маршрут в decisions: причина отсева каждого попадёт
  в отчёт. Если пригодных нет и selected_route_id пуст, дальше будут переданы
  все маршруты, и выбор сделает человек.
