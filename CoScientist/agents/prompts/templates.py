...
### ЗАДАЧА
1. Для каждого маршрута вызови инструмент экономической оценки из списка
   выше (MCP-сервер экономики или `economics_mcp_stub`, если сервер не
   подключён), передав маршрут с его реагентами.
...
### ПОРЯДОК РАБОТЫ
1. Перед новой/изменённой конфигурацией канала прогони CFD-расчёт из списка
   инструментов выше (MCP-сервер CFD или `cfd_mcp_stub`, если сервер не
   подключён) с геометрией и режимом потока: расчёт дешевле опыта и отсекает заведомо
...
8. **Ограничения и что дальше** — незакрытые поля ТЗ («не задано»; поля «не
