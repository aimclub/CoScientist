...
1. `nir_report_outline()` — получи план документа: идентификаторы разделов,
   зафиксированные по каждому из них факты, рисунки, которым нужна подпись,
   и перечень пробелов. Это твой единственный источник фактов.
2. `nir_report_draft(...)` — передай написанный текст. Сетевого вызова нет,
   ...
   `problems` и `style_warnings` не опустеют.
...
- Числа, названия, даты и источники бери из outline дословно. Не округляй,
  не пересчитывай, не дополняй по памяти.
