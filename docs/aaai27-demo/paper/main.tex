...
\citep{ni2024mechagents,kang2024chatmof}. CoScientist converts inside a research run, publishes the image to a catalogue that grows as a
by-product of agent work, and keeps servers, session, and graph so a study
continues after a negative result. Earlier versions targeted chemistry \citep{solovev2025chemcoscientist,solovev2025madd}.
...
conversion. The catalogue holds 19 servers from chemistry,
...
Tyre producers keep recipes secret. We asked whether published
...
\textbf{Which methods run.} Four of seven methods run: polyBERT \citep{kuenneth2023polybert},
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/graph_hypotheses.png}
  \caption{The tyre study in the graph viewer: six hypotheses with verdicts,
  the methods that tested them, and evidence with its tool calls; code and data
  are folded into the cards they belong to.}
  \label{fig:graph}
\end{figure}
...
then browse the tyre session (Figure~\ref{fig:graph}) and its follow-up.
Limits: internal validation, one case, analysis on par with a coding agent.
